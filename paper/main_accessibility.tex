...
Two observations frame whether 0.75 is ``enough.'' First, it is not comparable to
published numbers unless the split protocol is stated: our own detector reaches 0.86
...
0.859~\cite{exposureengine} — reported on professionally produced broadcast footage,
a different domain from self-captured club video — would be meaningless in either
direction. Second, per-frame mAP is not the quantity a sponsor report is built from.
...
and EMV figures are computed from. To validate it,
