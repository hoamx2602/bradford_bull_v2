...
%%  This includes FIGURES: raw video frames reveal the club instantly
%%  (scoreboards, crest, kit, club-TV watermark, opponents). Do not add
%%  qualitative frame figures unless scoreboard/crest/watermark are redacted
%%  and the user has approved the redaction (decision 2026-07-10).
%% ============================================================================
...
\caption{\textbf{What the detector actually faces in self-captured video:}
correct low-confidence detections (boxes; confidence under each crop) from one
night and one daylight fixture. Top: floodlit night play — small, dim,
motion-smeared marks on moving players. Bottom: heavy motion blur, sub-0.1\%-of-frame
scale, and fold/occlusion deformation. Sponsor marks at these scales are inherently
illegible, which is precisely why a hard confidence gate would delete genuine
exposure; the pipeline instead keeps them and lets visibility weighting and the
minimum-duration rule (\S\ref{sec:emv}) suppress spurious ones. (Jersey text outside
one detection is pixelated for club anonymity.)}
\label{fig:hard}
\end{figure}

Three observations frame whether 0.75 is ``enough.'' First, the operating domain is
harder than professionally produced broadcast: Fig.~\ref{fig:hard} shows
representative low-confidence detections — small, motion-blurred, deformed — that
are routine in club-captured footage. Second, the number is not comparable to
