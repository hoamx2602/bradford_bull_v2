...
\title{LogoLens: A Vision AI Tool to Measure Kit Sponsor\\
...
and measured per-slot exposure is strongly non-uniform, with two shoulder slots
...
As a qualitative evaluation, we visually audited the per-track attribution overlay of
every one of the nine full-match deployment videos, sampling frames late in each
overlay window so that temporal votes had stabilised. Five matches show correct
target/opponent assignment throughout the audited frames; three work overall but
exhibit per-track errors in both directions (two long-range night-time club feeds and
one fixture in which both teams wore dark kits); one match fails systematically and is
analysed as a failure case in \S\ref{sec:exp-counterfactual}. The audit also localises
\emph{where} the method degrades — distance, night lighting, and low kit contrast —
which fixes the conditions a quantitative evaluation must stratify by.
\TODOnum{Report precision/recall of team attribution on a labelled clip sample
stratified by the three conditions above (day/night, near/far, kit contrast) —
required before submission; the audit above is evidence of behaviour, not a
...
value: the overlay audit of \S\ref{sec:exp-teamattr} flags the match with the
highest exclusion rate (78\%) as failing systematically — under that fixture's night
floodlighting, the fused classifier assigned the target club's own white kit to the
opponent class, despite reference features that a luminance check confirms are
correct. We report that match as a failure case (\S\ref{sec:discussion}), not
