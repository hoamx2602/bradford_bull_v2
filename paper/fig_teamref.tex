...
\begin{tikzpicture}[node distance=3.5mm and 2.5mm]
  % tier cascade
  \node[blk, text width=23mm] (t1) {Tier 1\\manual refs /\\official kit image};
  \node[blk, right=of t1, text width=23mm] (t2) {Tier 2\\kit-anchor\\similarity};
  \node[blk, right=of t2, text width=23mm] (t3) {Tier 3\\luminance\\heuristic};
  \draw[arr] (t1) -- node[above, font=\tiny\itshape]{missing} (t2);
  \draw[arr] (t2) -- node[above, font=\tiny\itshape]{missing} (t3);
