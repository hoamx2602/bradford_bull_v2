...
class Sam3Labeler:
    ...
    def _run_mock(self, frame: np.ndarray, roster: Roster) -> list[RawDet]:
        ...
        for k in range(1 + seed % 2):
            ...
            dets.append(RawDet(brand, (float(x0), float(y0),
                                       float(x0 + bw), float(y0 + bh)),
                               conf, track_id=(seed + k) % 50))
...
def to_yolo_line(d: RawDet, roster: Roster, w: int, h: int) -> str:
    x0, y0, x1, y1 = d.xyxy
    cx, cy = (x0 + x1) / 2 / w, (y0 + y1) / 2 / h
    bw, bh = (x1 - x0) / w, (y1 - y0) / h
    return f"{roster.brand_to_id[d.brand]} {cx:.6f} {cy:.6f} {bw:.6f} {bh:.6f}"
...
def run(video: Path, logos: Path, out: Path, backend: str, weights: str,
        ...
        text_backend: str, text_weights: str, device: str | None = None) -> None:
    ...
    while True:
        ...
        (lbl_dir / f"{stem}.txt").write_text(
            "\n".join(to_yolo_line(d, roster, w, h) for d in dets))
